...
const CareerInterestsClient = () => {
  ...
  const scores = useMemo(() => {
    if (scoreString) {
      ...
      const results: { [key: string]: number } = {}; // ✅ scores의 타입을 명시적으로 지정
    ...
    return {};
  }, [interestCodes, scoreString]); // 의존성 배열에 searchParams가 아니라 파라미터로 받은 interestCodes와 scoreString

  const primaryCode = Object.keys(scores).sort((a, b) => scores[b] - scores[a])[0];

  const codeMap: { [key: string]: { name: string; color: string; detail: string } } = { // ✅ codeMap 타입 명시적 지정
