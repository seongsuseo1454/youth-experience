...
// 임시 라우트 정의 (사용자가 제공한 FancyMenu 컨텍스트 기반)
const R = { 
    level: '/career/level',
    consultant: '/career/consultant',
    interests: '/career/interests', // 다음 단계
...
export default function WheelDemoPage() {
  ...
  const handleNavigation = (pickedId: number) => {
    const pickedName = avatars.find(a => a.id === pickedId)?.name || 'Unknown';
    console.log(`[NAVIGATE] 최종 선택: ${pickedName}. 다음 단계(${R.interests})로 이동합니다.`);
    // '관심분야 선택' 단계로 이동
    router.push(R.interests);
  }
...
function Wheel({avatars, onPick, onNavigate}: {avatars:Av[]; onPick?:(a:Av)=>void; onNavigate?:(id:number)=>void}) {
    ...
    const [showModal, setShowModal] = useState(false); // ✅ 추가: 모달 표시 상태
    ...
    const spin = ()=>{
        ...
        setPickedId(null);
        setSpinning(true);
        setShowModal(false); // 회전 시작 시 모달 닫기
    ...
    return (
        <div style={{textAlign:'center'}}>
            ...
            <div style={{marginTop:36, display:'flex', gap:16, justifyContent:'center'}}>
                ...
                <button
                    onClick={()=>{ if(pickedId){ setShowModal(true); } }} // ✅ 수정: alert 대신 모달 표시
            ...
            {/* ✅ 추가: 커스텀 모달 */}
            {showModal && pickedName && (
                <div style={modalOverlayStyle} onClick={() => setShowModal(false)}>
                    <div 
                        style={modalContentStyle} 
                        onClick={(e) => e.stopPropagation()} // 모달 내부 클릭 시 닫히지 않게 방지
                    >
                        <h3 style={{fontSize: 24, fontWeight: 700, color: '#10B981', marginBottom: 12}}>상담사 선택 완료!</h3>
                        <p style={{fontSize: 20, color: '#374151', marginBottom: 24}}>
                            선택된 상담사: <b style={{color: '#2563EB'}}>{pickedName}</b>
                        </p>
                        <p style={{fontSize: 16, color: '#6B7280', marginBottom: 24}}>
                            선택된 아바타와 함께 다음 단계로 진행합니다.
                        </p>
                        <button
                            onClick={() => {
                                if (pickedId) {
                                    onNavigate?.(pickedId); // ✅ 수정: 라우팅 함수 호출
                                }
                                setShowModal(false);
                            }}
                            style={{
